...
def resolve_tags(investigation, incoming_tags):
    """Create or reuse tags for the investigation based on provided names."""
    ...
    for raw_tag in incoming_tags:
        ...
        existing = investigation.tags.filter(name__iexact=cleaned).first()
        if existing:
            resolved.append(existing)
            continue

        resolved.append(Tag.objects.create(investigation=investigation, name=cleaned))
...
def api_entities(request, investigation_id):
    ...
    if request.method == 'GET':
        entities = investigation.entities.prefetch_related('tags').all()
        ...
        return JsonResponse({
            'entities': [{
                ...
                'photo_url': entity.photo.url if entity.photo else None,
                'tags': [{
                    'id': str(tag.id),
                    'name': tag.name,
                    'color': tag.color
        ...
        try:
            ...
            if 'tags' in data:
                resolved_tags = resolve_tags(investigation, data.get('tags'))
                entity.tags.set(resolved_tags)
...
@login_required
@csrf_exempt
def api_entity_photo(request, investigation_id, entity_id):
    """Upload or delete an entity photo."""
    investigation = get_object_or_404(Investigation, id=investigation_id)
    entity = get_object_or_404(Entity, id=entity_id, investigation=investigation)
    ...
    if request.method == 'POST':
        ...
        entity.photo = file
        entity.save(update_fields=['photo', 'updated_at'])
    ...
    if request.method == 'DELETE':
        if entity.photo:
            entity.photo.delete(save=False)
            entity.photo = None
            entity.save(update_fields=['photo', 'updated_at'])

        return JsonResponse({'message': 'Photo supprimée'})
